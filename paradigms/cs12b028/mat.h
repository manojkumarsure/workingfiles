...
public:
	string expr;
	Error_handler(string expression)
	{
		expr=expression;
	}
	int bracketmatch()
	...
	int operatorverify()
	{
		...
		for(int i=0;i<expr.length();i++)
		{
			if(expr[i]=='(')
			{
				bracket_match=get_pair(i);
				if(bracket_match==i+1)
				{
					cout<<"error"<<endl;
					return -1;
				}
				values[i]=type(i,bracket_match);
...
public:
	...
	EvalObject mat;
	Matrix_Operations x;
	expressionEvaluator(string expression,EvalObject matrix)
	{
		expr=expression;
		mat=matrix;
	}
	EvalObject evaluate()
	{
		...
		for(int i=0;i<postfix.length();i++)
		{
			...
			if(postfix[i]=='*')
			{
				...
				b=eval_stack.top();
				eval_stack.pop();
				eval_stack.push(x.multiply(b,a));
			}
			if(postfix[i]=='+')
			{
				a=eval_stack.top();
