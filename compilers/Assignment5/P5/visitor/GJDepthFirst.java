...
public class GJDepthFirst<R,A> implements GJVisitor<R,A> {
   ...
   basicblock currblock;
	...
	public static Integer spillcount=0;
	int use=0;
	int linenumber=0;
   ...
   public R visit(NodeOptional n, A argu) {
      if ( n.present() )
      {
		...
		prevblock = currblock ;
		labelmap.put(a.toString(),linenumber);
         return a;
      }
      else
         return null;
...
// 		for(tempvariable i : sortlist )
// 		{
// 			System.err.println("TEMP "+i.tempnumber+"-"+i.Allotreg+"-"+i.start+":"+i.end);
// 		}
		templist = sortlist;
	}
	...
	public void newliveranges()
	{
		for(tempvariable i : alltemps)
		{
			if(i.start == 99999999)
			{
				i.start = defprocs.get(i.ProcedureName).start;
			}
			if(i.end == -1)
			{
				i.end = defprocs.get(i.ProcedureName).end;
			}
		}
		linearscan();
	}
...
//       newliveranges();
   ...
   public R visit(CJumpStmt n, A argu) {
      ...
      if(labelmap.get(a.toString())!=null)
      {
		int num = labelmap.get(a.toString());
		for(tempvariable i : alltemps)
		{
			if (i.end < linenumber && i.end >= num && i.start < num)
			{
				i.end  = linenumber;
			}
		}
   ...
   public R visit(Temp n, A argu) {
	  ...
	  if(use==0)
	  {
		{ if (x.start==99999999)x.start=linenumber;}
