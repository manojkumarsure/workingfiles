#Problem4:
#Applying linear regression and knn for mixture of guassians
...
#choosing two means and 3 random covariance matrices with each entry
#in covariance matrix is between 0 and 3
mean1=[0]*10
mean2=[3]*10
covariance=[]
covariance2=[]
...
list=[0]*10+[1]*42+[2]*48 #used for mixture probabilities
l1=[[0]*10]*1000
l2=[[0]*10]*1000

#based on the random choice that is made we choose a point to be taken from which covariance matrix
#so that the randomisation is ensured in the resulatant dataset also
...
#After generating the dataset we split the dataset into test and train
#As l1 and l2 has a random mixture of all 3 guassians we dont need to choose
#the test set randomly again
...
#for printing the generated data set into csv files
fp=open("DS2-train.csv","w")
fp.close()
fp=open('DS2-test.csv','w')
fp.close()
fp=open("DS2-train.csv","a")
for i in range(len(Train)):
	for j in range(1,11):
		print >>fp,str(Train[i][j])+',',
	print >>fp
fp.close()
fp=open('DS2-test.csv','a')
...
#calling linear and knn functions to check which performs better
linear()
knn(5) # argument is the no. of neighbours for knn